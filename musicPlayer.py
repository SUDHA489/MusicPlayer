...
def next_song():
    ...
    next_index = (current_selection[0] + 1) % len(songs)
...
def prev_song():
    global current_song
    current_selection = songlist.curselection()
    prev_index = (current_selection[0] - 1) % len(songs)
    songlist.selection_clear(0, END)
...
songlist.pack()
songlist.bind("<ButtonRelease-1>", on_song_select)
...
prev_btn = Button(control_frame, image=prev_btn_image, borderwidth=0, command=prev_song)
